...
def build_mining_detection(year):
    ...
    # Buffer water areas by 100m to catch riverbanks and small islands
    ...
    # ── 4. Mining detection: broad detection ──
    mining = (ndvi.lt(0.45)              # Catch revegetating/partial mining areas
              .And(bsi.gt(-0.05))         # Almost any non-forest soil
              .And(s2.select('B11').gt(700)))  # Low SWIR threshold

    # ── 5. Strict water exclusion ──
    mining = mining.And(land_only)                # JRC buffered + coastal
    mining = mining.And(ndwi.lt(-0.1))            # Clearly NOT water
    mining = mining.And(mndwi.lt(0.0))            # Clearly NOT water
    mining = mining.And(s2.select('B8').gt(s2.select('B3')))  # NIR > Green = land
